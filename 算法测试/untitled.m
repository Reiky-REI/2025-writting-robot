...
% 筛选非负角度
phi1_candidates = [phi1_1_deg, phi1_2_deg];
phi4_candidates = [phi4_1_deg, phi4_2_deg];

% 计算 phi1 的补角
phi1_complement = 180 - phi1_candidates;

% 找到 phi1 的补角和 phi4 相差最小的组合
min_diff = inf;
best_phi1 = 0;
best_phi4 = 0;
best_phi1_complement = 0;

for i = 1:2
for j = 1:2
if phi1_complement(i) >= 0 && phi4_candidates(j) >= 0
diff = abs(phi1_complement(i) - phi4_candidates(j));
if diff < min_diff
min_diff = diff;
best_phi1 = phi1_candidates(i); % 记录原始的 phi1
best_phi1_complement = phi1_complement(i); % 记录补角
best_phi4 = phi4_candidates(j); % 记录 phi4
end
end
end
end
...
% 输出最终结果
fprintf('phi1 的补角和 phi4 相差最小的两个角度为:\n');
fprintf('phi1: %.4f 度\n', best_phi1);
fprintf('phi1 的补角: %.4f 度\n', best_phi1_complement);
fprintf('phi4: %.4f 度\n', best_phi4);